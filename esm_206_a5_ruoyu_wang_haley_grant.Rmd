...
### A report prepared for ESM 206: Statistics and Data Analysis in Environmental Science and Management
...
Null Hypothesis ($\H_0_\$): The mean weights of Pacific giant salamanders from Pool, Cascade, and Side Channel are the same. Alternative Hypothesis (HA): At least two group means are NOT equal.
...
c) Population variances are equal. In the summary table, notice that the largest variance (Pool - `r round(channel_summary_table$var[2], 2)`) is less than the four times of the smallest variance (Side Channel - `r round(channel_summary_table$var[3], 2)`), which means the variances are close enough. Besides, based on the results from Levene's Test (P() > 0.05), there is no significant difference in variances between groups.
...
levene_test_e
# not significant, retain null hypothesis meaning that there is no significate difference between the variances between different groups. `r round(levene_test_e$[1], 3)`
```
