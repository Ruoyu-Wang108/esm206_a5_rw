...
This report compares the weights and abunance of Pacific giant salamanders in Mack Creek (near Blue River, Oregon). The area is composed of Clear Cut and Old Growth forrests. The Clear Cut sections were cut down around 1963, and the Old Growth coniferous forest is around 500 years old. Data related to their locations within Mack Creek is also studied. Within the two sections of forrests, salamanders are found in Cascades, Pools and Side-Channels. Salamanders found in Isolated Pools were not included in this report, as they the pools are separated from the channel. We are only interested in salamanders found in the creek.  
...
```{r result_c_chisquare_test, include = FALSE}
...
ggplot(mack_creek_e, aes(x = unittype, y = weight)) +
  ...
  labs(y = "Weight",
       x = "Location",
       title = "Salamander weights by location",
       subtitle = "2017") + 
...
```{r ruoyu_result_d}
salam_d <- mack_creek_df %>%
  filter(year == 2017) %>% 
  filter(section %in% c("CC","OG")) 

forest_summary_table <- salam_d %>% 
  group_by(section) %>% 
  summarize(
