...
First, we made our hypothesises. 
Null Hypothesis (H0): The mean weights of Pacific giant salamanders from pools, cascades, and side channels are the same. Alternative Hypothesis (HA): At least two group means are NOT equal.

Second, we check whether the data satisify the assumptions for ANOVA: 

1) The sample distribution should be normal. Through the beeswarm plot we could observe that most weights were concentrated at 0-6g. The data was presented as stew to the left side. However, the sample sizes within each groups (pools=`r channel_summary_table$count[2]`, cascades=`r channel_summary_table$count[1]`), and side channel=`r channel_summary_table$count[3]`) are all more than 30, which could be considered as large enough. So the distributions of the sample statistics will be normally distributed.

2) Samples are independent. Due to the different locations of these three channels, the sample data collected from different places should be independent.
...
The actual difference of mean weights between salamanders found in Pools and salamanders equals
