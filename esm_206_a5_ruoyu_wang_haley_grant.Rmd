...
# Pacific giant salamanders in Mack Creek: comparing abundance and weight against forest types and channel sections
...
Around 1963, sections of forests near Mack Creek, Oregon were clear cut, meaning the trees were uniformly cut down (Gregory 2016) (Wikipedia). In 1993, Stanley V. Gregory and researchers at the Andrews Forest LTER Site began including Pacific giant salamanders (Dicamptodon tenebrosus) in their study called "Aquatic Vertebrate Population Study in Mack Creek, Andrews Experimental Forest, 1987 to present." The researchers recorded the weights of salamanders (g) and made note of their locations within the area. The two forest sections of Mack Creek include clear cut sections as well as 500-year-old coniferous old growth forests. Within the two sections of forests, salamanders are found in cascades, pools and side channels. This report does not include salamanders found in isolated pools, as these areas are separated from the creek. Studying the Pacific giant salamander population in this area is relevant to understanding how new forests- the result of clear cutting- affect local wildlife populations, and how these effects compare to populations living in old growth forests. 
...
In general, annual counts of salamanders were greater in old growth sections than in clear cut sections of the forest, but from 2012 to 2017, counts in clear cut sections surpassed counts in old growth sections. (Figure.3).
...
```
***Figure. 3*** *Annual counts of salamanders in Mack Creek from 1993 to 2017. Counts in clear cut forests (orange dotted line) tended to be less than counts in old growth forests (blue line with trianges) until 2012. Data: Andrews Forest LTER Site.*
...
There are no significant differences in forest section (clear cut / old growth) based on which channel type salamanders are found ($\chi$^2^(`r my_salam_chi$parameter`) = `r round(my_salam_chi$statistic,2)`, *p* (`r round(my_salam_chi$p.value,3)`) > 0.05).

These results indicate that the proportions of salamanders in each section and channel type, as outlined in Table 1, are to be expected if forest section is not a determining factor in where salamanders were located in Mack Creek. 
...
***Figure. 5*** *The grey points showed the mean weights within each channel groups (Side channel - `r round(channel_summary_table$mean[3], 2)`, Pool - `r round(channel_summary_table$mean[2], 2)`, and Cascade - `r round(channel_summary_table$mean[1], 2)` ), the error bar indicates the standard diviation of the data (Side channel - `r round(channel_summary_table$sd[3], 2)`, Pool - `r round(channel_summary_table$sd[2], 2)`, and Cascade - `r round(channel_summary_table$sd[1], 2)`). Data: Andrews Forest LTER Site.*
...
- channel locations and sampling sections are independent (Table.1)
...
- Within three channels, the mean salamander weight differed significantly between Side Channel and Pool, but not with other groups (Figure.5).
...
Clearcutting. Wikipedia. https://en.wikipedia.org/wiki/Clearcutting. Accessed 12/03/19. 
